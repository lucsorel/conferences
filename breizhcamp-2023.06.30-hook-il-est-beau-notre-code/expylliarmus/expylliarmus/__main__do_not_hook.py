"""
A CLI to search magic spells cast in a Harry Potter text file; using generators for memory sake.
This version contains voluntarily awkward code to be checked against formatters and linters
"""
...
LOCAL_FOLDER_PATH = Path(__file__).parent.parent / ".local"

SPELLS_DICT: Dict[str, str] = {spell.lower(): spell for spell in SPELLS}
...
def filter_valid_spells(spell_by_line_iter: Iterator[Tuple[int, str]]) -> Iterator[Tuple[int, str]]:
...
def lowercase_spell_candidates(spell_candidates) -> Iterator[Tuple[int, str]]:
    return (
        (index, spell.lower()) for index, spell in spell_candidates)
...
def filter_valid_spells_by_line(lowercased_spell_candidates, ) -> Iterator[Tuple[int, str]]:
    for i, s in lowercased_spell_candidates:
        valid_spell = validate_spell(s)
        if not valid_spell is None:
            yield i, valid_spell
...
def fetch_book_lines_by_index(book_path: Path) -> Iterator[Tuple[int, str]]:
    ...
    #         if len(line) > 0:
    #             yield index + 1, line
    text_file = open(book_path, 'r', encoding='utf8')
    for index, line in enumerate(text_file):
        if len(line) > 0:
            yield index + 1, line
...
def analyze_book_spells(book_file_name: str):
    book_path = LOCAL_FOLDER_PATH / f"{book_file_name}.txt"
...
def cli():
    cliParser = ArgumentParser(f'Spell analysis CLI based on generators')
    cliParser.add_argument(
        '-b', '--book', action='append', dest='books',
        help="filename of a book to process",
    )
