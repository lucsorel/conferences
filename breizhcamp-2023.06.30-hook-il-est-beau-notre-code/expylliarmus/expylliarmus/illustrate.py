...
def analyze_spells_distribution(csv_path: Path, book_file_name: str):
    with open(csv_path, 'r') as spell_by_line_file:
        ...
        #     lines.append(row[0])
        #     spells.append(row[1])
        lines, spells = zip(*((int(row[0]), row[1]) for row in spell_by_line_csv), strict=True)
        ...
        spell_names, spell_occurrences = zip(*spell_counter.most_common()[::-1], strict=True)
        spell_ticks = range(len(spell_names))
        spells_bar_axes.barh(spell_ticks, spell_occurrences)
        spells_bar_axes.set(
            xlabel='spell occurrences',
            ylabel='spells',
            title=f'Occurrences of spells cast in {book_file_name}',
        )
        ...
        spell_lines_axes.set(
            xlabel='line index',
            ylabel='number of cast spells',
            title=f'Distribution of spells cast in {book_file_name}',
        )
        spell_lines_axes.tick_params(axis='x', which='major', labelsize=6)
        fig.tight_layout()
        fig.savefig(str(csv_path.parent / f'{csv_path.name}.png'), format='png')
